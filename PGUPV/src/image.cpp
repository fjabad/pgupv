...
PGUPV::GeoKeyDirectory readGeoKeyDirectory(const uint16_t* data) {
	...
	void readTiePoints(const void* data, const uint length, PGUPV::GeoTiffMetadata & md) {
		...
		for (uint i = 0; i < length / sizeof(double); i += 6) {
			...
			tp.imagePoint = glm::vec3(d[i], d[i + 1], d[i + 2]);
			tp.modelPoint = glm::vec3(d[i + 3], d[i + 4], d[i + 5]);
	...
	bool Image::ImageImpl::loadSimple(const std::string & filename, const FREE_IMAGE_FORMAT fileType) {
		...
		if (fileType == FIF_TIFF && FreeImage_GetMetadataCount(FIMD_GEOTIFF, freeimageImage) > 0) {
			GeoTiffMetadata gtMetadata;
			GeoKeyDirectory gkd;
			const double* geoDoubleParams = nullptr;
			const char* asciiParams = nullptr;
			std::cout << "# tags: " << FreeImage_GetMetadataCount(FIMD_GEOTIFF, freeimageImage) << "\n";
			FITAG* tag = nullptr;
			FIMETADATA* mdhandle = nullptr;
			mdhandle = FreeImage_FindFirstMetadata(FIMD_GEOTIFF, freeimageImage, &tag);
			if (mdhandle) {
				do {
					// process the tag
					auto tagName = std::string(FreeImage_GetTagKey(tag));
					std::cout << tagName << " = ";
					std::cout << FreeImage_TagToString(FIMD_GEOTIFF, tag) << "\n";
				...
				gtMetadata.widthPixels = FreeImage_GetWidth(freeimageImage);
				gtMetadata.heightPixels = FreeImage_GetHeight(freeimageImage);
				geoTiffMetadata = gtMetadata;
