...
PGUPV::Image::Image(Image&& other) noexcept
...
Image::ImageImpl::ImageImpl(const std::string& filename)
	: _data(nullptr), _filename(filename), freeimageImage(nullptr), freeimageMultiImage(nullptr) {
	initLib();
	if (!load(filename))
...
Image PGUPV::Image::convert8BPPGrayTo24BPPGray(const Image& src)
{
...
bool PGUPV::Image::saveHDR(const std::string& filename, uint32_t width, uint32_t height, uint32_t bpp, const float* bytes)
{
	ERRT("No implementado. Habla con Paco");
}
