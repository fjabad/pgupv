...
bool OrbitCameraHandler::mouse_move(const MouseMotionEvent& e) {
	...
	if (e.state) {
		...
		// Bot�n izquierdo: rotar
		if (e.state & 1)
			moveRelative(0.0, (float)-e.xrel / width, (float)e.yrel / width);
		else
			// Bot�n derecho: acercar/alejar la c�mara
			if (e.state & 4)
...
/**
Guarda el estado actual de la c�mara en el flujo indicado
*/
...
static json readJsonFromFile(std::istream& stream) {
	...
	if (j.empty()) {
		try
		{
			...
			if (stream.eof()) ERR("No se ha podido abrir el fichero para leer el estado de la c�mara");
			j = json::parse(str);
		}
		catch (const std::exception& e)
		{
			ERRT(std::string("Error leyendo el fichero del estado de la c�mara: ") + e.what());
		}
	}
...
/**
restaura el estado de la c�mara al que se encuentra en el flujo
*/
void OrbitCameraHandler::loadStatus(std::istream& stream) {
	json j = readJsonFromFile(stream);

	if (!j["orbitcamera"]) {
		ERR("Tipo de c�mara incompatible. Usando OrbitCameraHandler");
		return;
	}
...
void XYPanZoomCamera::loadStatus(std::istream& stream) {
	...
	if (!j["xypanzoomcamera"]) {
		ERR("Tipo de c�mara incompatible. Usando ZYPanZoomCamera");
		return;
	}
	_width = j["width"];
...
void WalkCameraHandler::update(uint64_t ms) {
...
// Establece los �ngulos de la direcci�n de la vista (ver definici�n
// en el constructor
...
void WalkCameraHandler::loadStatus(std::istream& stream) {
	...
	if (!j["walkcamera"]) {
		ERR("Tipo de c�mara incompatible. Usando WalkCamera");
		return;
	}
	_yaw = j["yaw"];
	_pitch = j["pitch"];
