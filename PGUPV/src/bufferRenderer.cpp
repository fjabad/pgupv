...
static std::vector<std::string> showDepthStencilVert{
  "#version 420 core",
  "in vec3 position;"
  "void main() {"
...
static std::vector<std::string> showDepthStencilFrag{
	"#version 420 core",
...
BufferRenderer::BufferRenderer(Window& window)
	...
	_shader.loadStrings(showDepthStencilVert, showDepthStencilFrag);
	_shader.compile();
