...
private:
	...
	std::shared_ptr<Program> zshader, renderProgram;
...
bool MyRender::reload() {
	...
	renderProgram = std::make_shared<Program>();

	// Definimos la posición de los atributos
	renderProgram->addAttributeLocation(Mesh::VERTICES, "position");
	renderProgram->addAttributeLocation(Mesh::NORMALS, "normal");

	renderProgram->connectUniformBlock(mats, UBO_GL_MATRICES_BINDING_INDEX);
	renderProgram->connectUniformBlock(luces, UBO_LIGHTS_BINDING_INDEX);
	renderProgram->replaceString("$" + UBOMaterial::blockName, UBOMaterial::definition);

	renderProgram->loadFiles(App::exercisesDir() + "p9/p9");
	renderProgram->compile();

	// Localización del uniform con la matriz de sombra
	shadowMatrixLoc = renderProgram->getUniformLocation("shadowMatrix");

	// Instalamos el shadow map en la unidad de textura 3
	renderProgram->use();
	GLint textureUnit = renderProgram->getUniformLocation("depthTexture");
